...
def eval_prepare(args):
    ...
    if args.test_type == 'intra':
        ...
        if character_names[0].endswith('_m'):
            ...
            file_id = [[args.target_bvh], [args.input_bvh]]
            src_id = 1
        else:
            character = [[character_names[0]], [character_names[1]]]
            file_id = [[args.input_bvh], [args.target_bvh]]
            src_id = 0
    else:
...
def main():
    ...
    for i, character_group in enumerate(character_names):
        ...
        input_motion.append([input_group, list(range(len(character_group)))]) # clip first two
        print(input_group.shape)
